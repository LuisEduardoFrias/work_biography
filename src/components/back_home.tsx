...
export default function BackHome({ position }: { position: 'left' | 'right' }) {
  ...
  if (position === 'right') {
    return (
      <button className="z-50 absolute top-0 right-0 bg-[linear-gradient(45deg,gray_40%,var(--base)_60%)] rounded-bl-[20px] rounded-tr-[20px] shadow-[-2px_2px_4px_0_gray]" onClick={() => handleClick()}  >
  ...
  return (
    <button className="z-50 absolute left-0 top-0 bg-[linear-gradient(-45deg,gray_40%,var(--base)_60%)] rounded-br-[20px] rounded-tl-[20px] shadow-[2px_2px_4px_0_gray]" onClick={() => handleClick()}  >
